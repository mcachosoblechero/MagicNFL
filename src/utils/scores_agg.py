...
def process_team_perf_match(records, games_results):
    ...
    team_match_perf = {
        ...
        'numQBOOP': len(records) - records.did_qb_stay_in_pocket.sum(),
        'percQBOOP': ((len(records) - records.did_qb_stay_in_pocket.sum()) / len(records)) * 100.0,
        'numLinemenFail': records.have_linemen_failed.sum(),
        'percLinemenFail': (records.have_linemen_failed.sum() / len(records)) * 100.0,
